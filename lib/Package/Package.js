// Generated by CoffeeScript 1.6.3
(function() {
  var Finder, Helpers, Info, Package, fs, path;

  Info = require('module-info');

  fs = require('fs');

  path = require('path');

  Finder = require('fs-finder');

  Helpers = require('../Helpers');

  Package = (function() {
    Package.SUPPORTED = ['js', 'json', 'ts', 'coffee'];

    Package.prototype.basePath = null;

    Package.prototype.skip = false;

    Package.prototype.application = null;

    Package.prototype.base = null;

    Package.prototype.style = null;

    Package.prototype.modules = null;

    Package.prototype.names = null;

    Package.prototype.run = null;

    function Package(basePath) {
      this.basePath = basePath;
      this.basePath = path.resolve(this.basePath);
      this.modules = [];
      this.names = {};
      this.run = [];
    }

    Package.prototype.getBasePath = function() {
      return this.basePath + (this.base === null ? '' : '/' + this.base);
    };

    Package.prototype.getPath = function(_path) {
      return path.resolve(this.getBasePath() + '/' + _path);
    };

    Package.prototype.expandPaths = function(paths) {
      var result, _i, _len, _path;
      result = [];
      for (_i = 0, _len = paths.length; _i < _len; _i++) {
        _path = paths[_i];
        if (_path.match(/^http/) === null) {
          _path = this.getPath(_path);
          if (fs.existsSync(_path) && fs.statSync(_path).isFile()) {
            result.push(_path);
          } else {
            result = result.concat(Finder.findFiles(_path));
          }
        } else {
          result.push(_path);
        }
      }
      return result.filter(function(el, pos) {
        return result.indexOf(el) === pos;
      });
    };

    Package.prototype.setApplication = function(application) {
      this.application = application;
      this.application = this.getPath(this.application);
      return this;
    };

    Package.prototype.setStyle = function(fileIn, fileOut, dependencies) {
      if (dependencies == null) {
        dependencies = null;
      }
      fileIn = this.getPath(fileIn);
      fileOut = this.getPath(fileOut);
      if (!fs.existsSync(fileIn)) {
        throw new Error('File ' + fileIn + ' does not exists.');
      }
      if (dependencies !== null) {
        dependencies = this.expandPaths(dependencies);
      }
      this.style = {
        "in": fileIn,
        out: fileOut,
        dependencies: dependencies
      };
      return this;
    };

    Package.prototype.addModule = function(name) {
      var found, main, paths, pckg, _i, _j, _k, _len, _len1, _len2, _path;
      found = false;
      if (name[0] === '/') {
        if (fs.existsSync(name)) {
          found = true;
          if (fs.statSync(name).isDirectory()) {
            pckg = new Info(name);
            main = pckg.getMainFile();
            if (main !== null) {
              this.registerModule(pckg.getName(), main);
            }
            this.registerModule(pckg.getName() + '/package.json', pckg.getPackagePath());
          } else {
            pckg = Info.fromFile(name);
            this.registerModule(pckg.getModuleName(name), name);
          }
        } else {
          paths = Finder.findFiles(name);
          if (paths.length > 0) {
            found = true;
            for (_i = 0, _len = paths.length; _i < _len; _i++) {
              _path = paths[_i];
              this.addModule(_path);
            }
          }
        }
      }
      if (found === false) {
        _path = Helpers.getCoreModulePath(name);
        if (_path !== null) {
          found = true;
          this.registerModule(name, _path);
        }
      }
      if (name[0] === '.' && found === false) {
        _path = this.getPath(name);
        if (fs.existsSync(_path)) {
          found = true;
          pckg = Info.fromFile(_path);
          name = pckg.getModuleName(name).replace(new RegExp('^' + pckg.getName() + '\/'), '');
          this.registerModule(name, _path);
        } else {
          paths = Finder.findFiles(_path);
          if (paths.length > 0) {
            found = true;
            for (_j = 0, _len1 = paths.length; _j < _len1; _j++) {
              _path = paths[_j];
              _path = path.relative(this.getBasePath(), _path);
              this.addModule('./' + _path);
            }
          }
        }
      }
      if (found === false) {
        _path = this.getPath('./node_modules/' + name);
        if (fs.existsSync(_path)) {
          found = true;
          pckg = Info.fromFile(_path);
          this.registerModule(pckg.getModuleName(_path), _path);
        } else {
          paths = Finder.findFiles(_path);
          if (paths.length > 0) {
            found = true;
            for (_k = 0, _len2 = paths.length; _k < _len2; _k++) {
              _path = paths[_k];
              _path = path.relative(this.getBasePath() + '/node_modules', _path);
              this.addModule(_path);
            }
          }
        }
      }
      if (found === false) {
        throw new Error('Module ' + name + ' was not found.');
      }
      return this;
    };

    Package.prototype.registerModule = function(name, data) {
      this.modules.push(data);
      return this.names[name] = this.modules.length - 1;
    };

    Package.prototype.addAlias = function(original, alias) {
      original = this.resolveRegisteredModule(original);
      if (original === null) {
        throw new Error('Module ' + original + ' is not registered.');
      }
      this.registerModule(alias, "`module.exports = require('" + original + "');`");
      return this;
    };

    Package.prototype.addToAutorun = function(name) {
      var file, files, fullName, _i, _len;
      fullName = this.resolveRegisteredModule(name);
      if (fullName === null) {
        fullName = path.resolve(this.getBasePath(), name);
        if (!fs.existsSync(fullName)) {
          files = Finder.findFiles(fullName);
          if (files.length === 0) {
            throw new Error('Module or library' + name + ' was not found.');
          }
          for (_i = 0, _len = files.length; _i < _len; _i++) {
            file = files[_i];
            this.addToAutorun(file);
          }
          return this;
        }
      }
      this.run.push(fullName);
      return this;
    };

    Package.prototype.resolveRegisteredModule = function(name) {
      var ext, _i, _j, _len, _len1, _ref, _ref1;
      if (typeof this.names[name] !== 'undefined') {
        return name;
      }
      _ref = Package.SUPPORTED;
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ext = _ref[_i];
        if (typeof this.names[name + '.' + ext] !== 'undefined') {
          return name + '.' + ext;
        }
      }
      _ref1 = Package.SUPPORTED;
      for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
        ext = _ref1[_j];
        if (typeof this.names[name + '/index.' + ext] !== 'undefined') {
          return name + '/index.' + ext;
        }
      }
      return null;
    };

    return Package;

  })();

  module.exports = Package;

}).call(this);
