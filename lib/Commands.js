...
(function() {
  var Commands, Compiler, EventEmitter, Finder, Q, express, fs, mime, ncp, path, watch,
  ...
  Commands = (function(_super) {
    ...
    Commands.prototype.logger = null;

    function Commands(simq, logger) {
      this.simq = simq;
      this.logger = logger != null ? logger : null;
    }

    Commands.prototype.log = function(message) {
      if (this.logger !== null) {
    ...
    Commands.prototype.server = function(prefix, main, routes, port) {
      ...
      for (route in routes) {
        ...
        if (fs.statSync(_path).isDirectory()) {
          this.log('Mapping ' + route + ' to directory ' + _path);
          app.use(route, express["static"](_path));
        } else {
          (function(data) {
            _this.log('Mapping ' + data.route + ' to ' + data.path);
      ...
      for (name in _ref) {
        ...
        if (pckg.skip === false) {
          ...
          (function(pckg) {
            if (pckg.target !== null) {
              _path = path.relative(_this.simq.basePath, pckg.target);
              _path = path.normalize(base + '/./' + _path);
              _this.log('Mapping ' + _path + ' to ' + pckg.name + ' package');
            ...
            if (pckg.style !== null) {
              ...
              _path = path.normalize(base + '/./' + _path);
              _this.log('Mapping ' + _path + ' to ' + pckg.name + ' package styles');
    ...
    Commands.prototype.watch = function() {
      ...
      return watch.watchTree(this.simq.basePath, {}, function(file, curr, prev) {
        if (typeof file === 'string' && file.match(/~$/) === null && file.match(/^\./) === null && ignore.indexOf(path.resolve(file)) === -1) {
          _this.log('Rebuilding from ' + file);
