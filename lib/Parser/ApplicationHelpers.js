  ...
  ApplicationHelpers = (function() {
    ...
    ApplicationHelpers.findDependentModules = function(from) {
      ...
      required(from, {
        ...
        var dep, ext, file, info, pckg, result, _i, _j, _len, _len1, _ref;
        if (e) {
          ...
          result.files = result.files.filter(function(el, pos, self) {
            return self.indexOf(el) === pos;
          });
          result.core = result.core.filter(function(el, pos, self) {
            return self.indexOf(el) === pos;
          });
          _ref = result.files;
          for (_j = 0, _len1 = _ref.length; _j < _len1; _j++) {
            file = _ref[_j];
            pckg = _this.findNodePackage(file);
            if (pckg !== null) {
              pckg = JSON.parse(fs.readFileSync(pckg, {
                encoding: 'utf8'
              }));
              info = _this.getModuleInfo(file, pckg);
