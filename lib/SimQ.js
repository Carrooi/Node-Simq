...
(function() {
  ...
  SimQ = (function() {
    ...
    SimQ.prototype.build = function() {
      ...
      fs.writeFileSync(this.basePath + '/' + config.application, this.parser.parseApplication(config));
      if (config.style && config.style["in"] && config.style.out) {
        this.parser.parseStyles(config.style["in"], function(content) {
          return fs.writeFileSync(_this.basePath + '/' + config.style.out, content);
        });
      }
      return this;
    };
