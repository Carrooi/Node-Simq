...
(function() {
  var Configurator, Parser, Q, SimQ, express, fs, mime, ncp, path, watch;
  ...
  SimQ = (function() {
    ...
    function SimQ(configPath, basePath) {
      ...
      this.config = new Configurator(this.configPath);
      this.parser = new Parser(this, this.basePath);
    }

    SimQ.prototype.server = function() {
      var app, base, config, data, main, name, pckg, route, _fn, _path, _ref, _ref1,
        _this = this;
      app = express();
      ...
      _fn = function(pckg) {
        if (_this.hasPackageApplication(pckg.name)) {
          if (_this.v) {
            console.log('Mapping file \'' + path.resolve(pckg.application) + '\' to \'' + base + path.normalize(pckg.application) + '\'');
          }
          app.get(base + path.normalize(pckg.application), function(req, res) {
        ...
        if (_this.hasPackageStyles(pckg.name)) {
          if (_this.v) {
            console.log('Mapping file \'' + path.resolve(pckg.style.out) + '\' to \'' + base + path.normalize(pckg.style.out) + '\'');
          }
          return app.get(base + path.normalize(pckg.style.out), function(req, res) {
    ...
    SimQ.prototype.build = function() {
      ...
      _fn = function(pckg) {
        var basePath;
        basePath = pckg.base === null ? _this.basePath : _this.basePath + '/' + pckg.base;
        if (_this.hasPackageApplication(pckg.name)) {
          _this.buildApplication(pckg.name).then(function(content) {
            return fs.writeFile(basePath + '/' + pckg.application, content);
          });
        }
        if (_this.hasPackageStyles(pckg.name)) {
          return _this.buildStyles(pckg.name).then(function(content) {
            return fs.writeFile(basePath + '/' + pckg.style.out, content);
          });
        }
      };
      for (name in _ref) {
