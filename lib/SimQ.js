...
(function() {
  var Configurator, Loader, Parser, SimQ, fs, less, ncp, watch, _path;
  ...
  SimQ = (function() {
    ...
    SimQ.prototype.build = function() {
      ...
      _fn = function(pckg) {
        if (pckg.application) {
          _this.parser.parseApplication(pckg, !config["debugger"].scripts).then(function(content) {
            return fs.writeFile(_this.basePath + '/' + pckg.application, content);
          });
        }
        if (pckg.style && pckg.style["in"] && pckg.style.out) {
          return _this.parser.parseStyle(pckg.style["in"], !config["debugger"].styles).then(function(content) {
            return fs.writeFile(_this.basePath + '/' + pckg.style.out, content);
          }, function(e) {
            e.type = 'Style ' + e.type;
            return less.writeError(e);
          }).done();
        }
      };
      for (name in _ref) {
